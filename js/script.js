...
function mapRowToProduct(row, headers) {
    ...
    headers.forEach((h, i) => {
        ...
        else if (header.match(/tipo|type/)) obj.tipo = val.toString().toLowerCase();
        ...
        } else if (header.match(/^ativo$/)) {
            obj.ativo = !(String(val).trim().toUpperCase() === 'N');
        }
    });
    obj.nome = obj.nome || "";
    obj.preco = obj.preco !== undefined ? obj.preco : 0;
    obj.categoria = obj.categoria || "outros";
    obj.tipo = obj.tipo || "";
...
function parseGvizResponse(resp) {
    ...
    const produtos = rows.map(r => mapRowToProduct(r.c, cols)).filter(p => p.ativo);
    allProducts.clear();
...
function criarCheckboxesTipo(containerElement, tiposSet) {
    vazioElemento(containerElement);
    ...
    Array.from(tiposSet).sort().forEach(tipo => {
        ...
        checkbox.value = tipo;
        checkbox.addEventListener('change', filtrarEExibirProdutos);
...
function criarSecoesCategorias() {
    ...
    for (const categoria of categoriasOrdenadas) {
        ...
        titleEl.textContent = categoria.charAt(0).toUpperCase() + categoria.slice(1);
        ...
        navLink.textContent = categoria.charAt(0).toUpperCase() + categoria.slice(1);
...
function filtrarLista(produtos, {
    categoria = '',
    genero = '',
    tipos = [],
    precoMin = 0,
    ...
    return produtos.filter(p => {
        ...
        if (termoBuscaLower) {
            const nomeProdutoLower = removerAcentos(p.nome.toLowerCase());
            if (!nomeProdutoLower.includes(termoBuscaLower)) {
                return false;
            }
        }
        return true;
...
async function loadAndRender() {
    ...
    try {
        ...
        try {
            ...
            const todosProdutosFallback = arr.map(obj => {
    ...
    filtroCategoria.addEventListener('change', (e) => {
        ...
        vazioElemento(filtroTipoContainer);
