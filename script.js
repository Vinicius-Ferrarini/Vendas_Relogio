...
function updateCartButtonText() {
  ...
  const cart = getCart();
  btn.textContent = cart.length > 0 ? `🟢 Ver Carrinho (${cart.length} ${cart.length > 1 ? 'itens' : 'item'})` : `🟢 Carrinho Vazio`;
}
...
function mapRowToProduct(row, headers) { 
    ...
    headers.forEach((h, i) => {
        ...
        else if (header.match(/pre[cç]o|price|valor/)) { if (typeof val === "number") { obj.preco = val; } else { const cleaned = String(val).replace(/[R$\s.]/g, "").replace(",", "."); const n = parseFloat(cleaned); obj.preco = isNaN(n) ? 0 : n; } }
        ...
        else if (header.match(/dataoferta/)) { if (cell && cell.f) obj.dataOferta = cell.f; else if (val && val.toString().includes('/')) obj.dataOferta = val.toString(); else obj.dataOferta = ""; }
        else if (header.match(/horaoferta/)) obj.horaOferta = val;
        else if (header.match(/^img\d?$|^imagem\d?$|^foto\d?$/)) { if (val) obj.images.push(val.toString()); }
    });
